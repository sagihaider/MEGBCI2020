...
for sub=1:1%length(indsub)
    for ses = 1:2
        ...
        fullfilenamein = fullfile(rootpathdatain,['sub-' num2str(indsub(sub))],['ses-' num2str(indsub(ses))],'meg',filenamein);
        
        sessfolderpath = fullfile(rootpathdataout,['Session_0' num2str(indsub(ses))]);
